...
namespace Frameworks_Mapper
{
    public class NISTCyberSecItems
    {
        ...
        public void SaveNISTCyberNIST80053Mapping(int NIST80053RevNo)
        {
            this.conn = new MySqlConnection(this.connStr);
            this.conn.Open();
                ...
                if(returnValue != null)
                {
                    ...
                    if (NIST80053RevNo == 4)
                    {
                        foreach (string nistControl in item.NIST80053Rev4Controls)
                        {
                            ...
                            this.saveNISTCyberSecNISTControlXREF(nistCyberId, nistControl, NIST80053RevNo);
                        }

                    }
                    else
                    {
                        foreach (string nistControl in item.NIST80053Rev5Controls)
                        {
        ...
        private void saveNISTCyberSecNISTControlXREF(int nistCyberSecId, string nistControlNo, int revisionNo = 4)
        {
            int nistId = 0;
            MySqlCommand cmd = new MySqlCommand();
            cmd.Connection = this.conn;
            ...
            MySqlParameter revisionParam = getSqlParameter(System.Data.DbType.Int32, System.Data.ParameterDirection.Input, "revision", revisionNo);
